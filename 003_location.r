...
dfx_loc <- merge(dfx1, loc, by.x="studyx", by.y="Study")
summary(dfx_loc)
...
##### order by median transmission time relative to symptom onset
...
p <- p + geom_point( data=summary_table_loc,
                     aes(x=Location1, y=mean_trs), col="purple", shape=17)
p
